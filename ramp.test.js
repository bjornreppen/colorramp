...
const rampSteps = 256
test('ramp viridis', () => {
  const c1 = hexToRgb('#440154')
  const c2 = hexToRgb('#fde725')
  const ramp = buildRamp(c1, c2, rampSteps)
  expect(ramp).toMatchSnapshot()
  console.log('<h2>Viridis</h2>' + ramp2html(ramp))
})

test('ramp inferno', () => {
  const c1 = hexToRgb('#000004')
  const c2 = hexToRgb('#fcffa4')
  const ramp = buildRamp(c1, c2, rampSteps)
  expect(ramp).toMatchSnapshot()
  console.log('<h2>Inferno</h2>' + ramp2html(ramp))
})
test('ramp plasma', () => {
  const c1 = hexToRgb('#0d0887')
  const c2 = hexToRgb('#f0f921')
  const ramp = buildRamp(c1, c2, rampSteps)
  expect(ramp).toMatchSnapshot()
  console.log('<h2>Plasma</h2>' + ramp2html(ramp))
})
